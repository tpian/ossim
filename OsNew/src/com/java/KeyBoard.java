...
public class KeyBoard extends Thread {
	...
	public void run() {
		while(true) {
			...
			try {
				GUI.timerCondition.await();        //等到时钟进程发出时钟中断，再开始执行下面操作
	...
	*/
	private void doWhatKeyBoardDoEverySecond() {
		...
		if(!ifKeyboardWork){                    //根据键盘的状态输出信息
			GUI.textArea.append("键盘状态：无进程请求" + "\n");
		}
		else {
			GUI.textArea.append("键盘状态：进程" + usingProcess.getID() + "请求，阻塞队列还有" + PCB.getBlockedQueue1Length() + "个进程:");
